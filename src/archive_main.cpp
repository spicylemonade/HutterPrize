...
struct TransformDecoder {
    // Header parsing
    static constexpr size_t MIN_HDR = 8; // HPZT v1
    static constexpr size_t HDR_V2 = 12; // HPZT v2 with dict checksum
    unsigned char hdr[12]; size_t hdr_pos = 0; size_t hdr_need = MIN_HDR;
    bool header_done = false; bool transforms = false; bool have_dict_crc = false; uint32_t dict_crc_in = 0;
    ...
    void reset() { hdr_pos = 0; hdr_need = MIN_HDR; header_done = false; transforms = false; have_dict_crc = false; dict_crc_in = 0; esc = ESC_NONE; digit_left = 0; }
    ...
    bool feed(const unsigned char* in, size_t n, FILE* fout, uint32_t& crc, uint64_t& written) {
        ...
        while (i < n) {
            ...
            if (!header_done) {
                ...
                // HPZT present
                unsigned ver = hdr[4]; unsigned flags = hdr[5];
                if (hdr_need == MIN_HDR && ver >= 2) { // need to read full v2 header
                    hdr_need = HDR_V2;
                    while (hdr_pos < hdr_need && i < n) hdr[hdr_pos++] = in[i++];
                    if (hdr_pos < hdr_need) return true;
                }
                ...
                if (have_dict_crc) {
                    ...
                    uint32_t dict_crc_here = hpzt_dict_checksum();
                    if (dict_crc_in != dict_crc_here) {
                        std::fprintf(stderr, "[ERROR] HPZT dictionary checksum mismatch (got 0x%08x, expected 0x%08x).\n", dict_crc_in, dict_crc_here);
                        return false;
                    }
                }
                continue;
            ...
            if (esc == ESC_NONE) {
                ...
                if (!sp.empty()) {
                    ...
                    crc = crc32_update(crc, sp.data(), sp.size()); written += sp.size();
                }
                esc = ESC_NONE;
            } else if (esc == ESC_NL) {
                ...
                if (!nl.empty()) {
                    ...
                    crc = crc32_update(crc, nl.data(), nl.size()); written += nl.size();
                }
                esc = ESC_NONE;
